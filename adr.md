...
> Wpisy ADR-9 i ADR-10 opisują decyzje *negatywne* (czego świadomie nie dodano).
...
| **Kontekst** | Zespół 2–3 os. pracujący jednocześnie nad frontendem i backendem. Celem jest minimalizacja przełączania kontekstu i błędów na granicy klient–serwer. |
...
| **Trade-offy** | Projekt zamyka się na ekosystem JS/TS. Monorepo wymaga konfiguracji workspace i dyscypliny w pilnowaniu granic pakietów. |
...
| **Kontekst** | Frontend i backend są w jednym repo w TypeScript. Wymagany jest type-safe kontrakt, który nie wymaga ręcznego utrzymywania typów po stronie klienta. |
...
| **Uzasadnienie** | End-to-end type safety „za darmo": zmiana procedury serwerowej jest natychmiast widoczna w typach klienta. Domena jest prosta i ma niewiele endpointów, więc elastyczność GraphQL nie jest potrzebna. |
...
| **Trade-offy** | Projekt wiąże się z Postgresem - kluczowe mechanizmy integralności są nieprzenośne na inne bazy. Drizzle jest młodszy i ma mniejszy ekosystem niż Prisma. |

## ADR-4 - Integralność rezerwacji wymuszana w bazie (`EXCLUDE`), nie w aplikacji

| Pole | Treść |
|---|---|
| **Decyzja** | Brak nakładających się rezerwacji tego samego zasobu wymusza się constraintem bazodanowym `EXCLUDE USING gist (resource_id WITH =, during WITH &&)`, a nie sprawdzeniem w kodzie aplikacji. |
...
| **Uzasadnienie** | Constraint w bazie jest atomowy i odporny na współbieżność z definicji - niemożliwe jest wstawienie kolidującego wiersza niezależnie od liczby równoczesnych transakcji. Eliminuje całą klasę błędów zamiast łatać ją w warstwie aplikacji. Anulowanie rezerwacji realizuje się jako **twarde usunięcie** wiersza (`DELETE`) - usunięty wiersz nie uczestniczy już w constraincie, więc termin jest realnie zwalniany bez potrzeby kolumny `status` ani klauzuli `WHERE`. |
...
| **Kontekst** | tRPC pełni rolę warstwy routingu, więc framework backendowy odpowiada tylko za serwer HTTP i middleware (CORS, cookies). Potrzebna jest lekka warstwa, ale nie „goła". |
...
| **Kontekst** | R4 wymaga logowania i ochrony endpointów rezerwacji. Projekt zakłada jeden kontrolowany serwer - nie potrzeba stateless tokenów. |
...
| **Uzasadnienie** | Better Auth jest framework-agnostyczny i TypeScript-native, ma gotowy adapter Drizzle (`provider: "pg"`) i montuje się w Hono jednym catch-all routem (`/api/auth/*`). Domyślnie używa sesji w cookies - co realizuje pierwotne założenie (łatwa rewokacja, httpOnly odporne na odczyt przez XSS) bez pisania auth od zera. Sprawdzanie sesji integruje się w `createContext` tRPC (`protectedProcedure`). |
| **Trade-offy** | Better Auth jest właścicielem tabel `user`/`session`/`account` - domena nie tworzy własnego modelu użytkownika, tylko referuje `reservation.userId -> user.id`. Dodaje się zależność i jej schemat (generowany przez CLI). Stan sesji w bazie utrudnia bezstanowe skalowanie poziome - przy tej skali nieistotne. |
...
| **Trade-offy** | TanStack Router jest młodszy niż React Router (krzywa uczenia, mniejszy ekosystem). SPA = brak SSR/SEO, co jest akceptowalne, bo aplikacja jest za logowaniem. |
...
| **Trade-offy** | Vitest celuje głównie w tooling Node - część testów backendu na Bun może wymagać uruchamiania pod Node lub świadomego wyboru runnera. Wbudowany runner Bun byłby szybszy i bezkonfiguracyjny, ale rozjechałby narzędzia front/back. Świadomie wybrano spójność (jedno narzędzie) kosztem natywnej prędkości Bun. |

## ADR-9 - Modularny monolit zamiast mikroserwisów *(decyzja negatywna)*

| Pole | Treść |
|---|---|
| **Decyzja** | Backend jako jeden modularny monolit. **Nie** wprowadza się mikroserwisów, message brokera (Kafka/RabbitMQ) ani osobnych serwisów komunikujących się po sieci. |
...
| Pole | Treść |
|---|---|
| **Decyzja** | Aplikację uruchamia się klasycznie (docker-compose, jeden host), z Postgresem po TCP. **Nie** kieruje się deploymentu na edge (Cloudflare Workers / Workerd). |
| **Kontekst** | Hono jest multi-runtime, więc kuszące jest deklarowanie „a potem przeniesienie na edge". Przeanalizowano konsekwencje dla warstwy danych. |
| **Alternatywy** | Deployment na Cloudflare Workers z Hyperdrive (pooling TCP) lub z driverem HTTP do Postgresa. |
| **Uzasadnienie** | Edge wymagałby albo vendor-specyficznej infrastruktury (Hyperdrive), albo drivera HTTP, który jest single-shot i **łamie interaktywno-transakcyjny model rezerwacji** z ADR-4. Przy R5 = docker-compose na jednym hoście to złożoność i lock-in za zero korzyści. Przenośność Hono traktuje się jako ubezpieczenie ryzyka runtime (ADR-5), nie jako cel deploymentowy. |
| **Trade-offy** | Rezygnuje się z potencjalnej niskiej latencji globalnej i skalowania na edge. Nieistotne dla aplikacji, której użytkownicy są zlokalizowani i która działa za logowaniem. |